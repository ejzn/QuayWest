...
#sidebar-menu li a:hover {color:<?php echo ($skin_color_populator == 'white' ?  "#000000" :  $colorPalette_brightwhite);?>;}
...
?>
<!-- SKIN STYLE SHEET -->
<link href="<?php bloginfo('template_directory'); ?>/skins/<?php echo $style_theme ?>/style.css" rel="stylesheet" type="text/css" />
<?php 
...
background: url(<?php bloginfo('template_directory'); echo "/images/" . $soc_icon_color; ?>) no-repeat;}
.soc a, soc a:hover{ opacity:1.0 ;}

<?php } else { ?>	
/* Social Media Icon  */
.googleplus, .googlemaps, .linkedin, .meetup, .phone, .pinterest, .rss, .sharethis, .tripadvisor, .tumblr, .twitter, .urbanspoon, .vimeo, .yelp, .youtube, .bookmark, .email, .facebook, .foursquare{
		background: url(<?php bloginfo('template_directory'); ?>/skins/<?php echo $style_theme ?>/images/sprites.png) no-repeat;}
<?php }
